...
class paired_icoll(icoll[Tuple[T1, T2]], Iterable[Tuple[T1, T2]]):
    ...
    def pairmap(self, f:Callable[[T1, T2], Any]):
        ...
        if self.empty:
            return icoll(self).map(_f) # just for consistency.
